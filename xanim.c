...
/* end SMR 2 */
 ...
 fprintf(stdout,"       Zr   Enable the remote buttons\n");
 fprintf(stdout,"\n");
...
#ifdef XA_EXPORT
 fprintf(stdout, "\n  E[eopts]  Export SubMenu\n");
 fprintf(stdout,"      Ea    Write audio to audio.raw.  Default off.\n");
#ifdef XA_EXPORT_QT
 fprintf(stdout,"      Ee    Write video to exported.mov.  Default off.\n");
#else
 fprintf(stdout,"      Ee    Write video to input/frameN.EXT. Default off.\n");
#endif
 fprintf(stdout,"      Ec#   Set codec for export.  Default 0.\n");
 fprintf(stdout,"               Codecs are 0 - RAW   1 - JPEG   2 - YUV 4:2:2 3 - YUV 4:2:0\n");
 fprintf(stdout,"      Eq#   JPEG quality from 0 - 100. Default 100.\n");
#endif
 ...
 for(i=1; i < argc; i++)
 {
   ...
   if ( ((in[0] == '-') || (in[0] == '+')) && (in[1] != '\0'))
   {
     ...
     while( (j < len) && (in[j] != 0) )
     {
       ...
       {
#ifdef XA_EXPORT
         case 'E':        /* export options sub menu */
	 {
	   xaULONG exit_flag = xaFALSE;
	   xa_skip_flag = xaFALSE;    /* don't skip frames when exporting */
	   xa_exit_flag = xaTRUE;     /* exit after playing once */
       xa_file_flag = xaTRUE;	   /* don't load input into RAM */
	   ...
	   while((exit_flag == xaFALSE) && (j < len))
	   {
		 ...
		 if(opt_on == xaTRUE)
		 {
		   XA_Video_Send2_Audio(XA_IPC_AUD_EXPORT,NULL,0,0,2000,0);
		 }
		 j++;
		 break;
		 ...
		 if(opt_on == xaTRUE)
		 {
		   export_data.export = 1;
		   j++; 
		   export_data.jpeg_quality = XA_Read_Int(in, &j);
		   xa_export_set_vid_compressor(xa_export_int_to_codec(1));
		 }
		 break;
		 ...
		 if(opt_on == xaTRUE)
		 {
		   ...
		   xa_export_set_vid_compressor(xa_export_int_to_codec(t));
		 }
		 break;
	      }
	    }
...
XA_MERGE_NOGO:
 ...
 /* Set Audio Ports */
  ...
  if(export_data.export)
  {
    ...
    export_data.frame = 0;
#ifdef XA_EXPORT_QT
    export_data.file = quicktime_open(export_data.path, 0, 1);
    quicktime_set_video(export_data.file, 
	1, 
	xa_imagex, 
	xa_imagey,
	10,	  /* framerate unknown */
	export_data.vid_compressor);
#endif
    export_data.depth = x11_depth;
    export_data.bits_per_pixel = x11_bits_per_pixel;
  }
#endif /* XA_EXPORT */
