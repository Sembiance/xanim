...
/****************************************************************************/
/**************** EXPORT AUDIO DEFINED ROUTINES******************************/
/****************************************************************************/
/* useful for backing out audio when device opens fails, etc */

void  Export_Audio_Init();
void  Export_Audio_Kill();
void  Export_Audio_Off();
void  Export_Audio_Prep();
void  Export_Audio_On();
xaULONG Export_Closest_Freq();

void XA_Export_Audio_Setup()
{
  XA_Audio_Init		= Export_Audio_Init;
  XA_Audio_Kill		= Export_Audio_Kill;
  XA_Audio_Off		= Export_Audio_Off;
  XA_Audio_Prep		= Export_Audio_Prep;
  XA_Audio_On		= Export_Audio_On;
  XA_Closest_Freq	= Export_Closest_Freq;
  XA_Set_Output_Port	= XA_NoAudio_Nop2;
  XA_Speaker_Tog	= XA_NoAudio_Nop;
  XA_Headphone_Tog	= XA_NoAudio_Nop;
  XA_LineOut_Tog	= XA_NoAudio_Nop;
...
/********** Export_Audio_Init **********************
 * Init Export Audio, init variables, open Audio output.
 *****/
void Export_Audio_Init()
{
  DEBUG_LEVEL2 fprintf(stderr,"Export_Audio_Init\n");

  if (xa_audio_present != XA_AUDIO_UNK) return;

  export_data.audio_fd = open("audio.raw", O_WRONLY|O_CREAT|O_TRUNC, 0666);
  if ( export_data.audio_fd < 0 )
...
/********** Export_Audio_Kill *************************************************
 * Stop Export Audio. Free up memory.
 *
 *****************************************************************************/
void Export_Audio_Kill()
{
...
/********** Export_Audio_Prep *************************************************
 * Startup Export Audio
 *
 *****************************************************************************/
void Export_Audio_Prep()
{
  DEBUG_LEVEL2 fprintf(stderr,"Export_Audio_Prep \n");
  if (xa_audio_status == XA_AUDIO_STARTED) return;
  ...
  else if (xa_snd_cur)
  {
    ...
    {
      if ( xa_audio_hard_freq != -1 )
        printf("WARNING: New output audio frequency: %d\n", xa_snd_cur->hfreq);
...
void Export_Audio_On()
{
  DEBUG_LEVEL2 fprintf(stderr,"Export_Audio_On\n");
  if (   (xa_snd_cur)
      && (xa_audio_present == XA_AUDIO_OK)
      && (xa_audio_status == XA_AUDIO_PREPPED) )
  { 
 ...
 *
 *****************************************************************************/
xaULONG Export_Closest_Freq(ifreq)
...
#ifdef XA_EXPORT
      if ( export_data.export ) write(export_data.audio_fd, xa_audio_ring->buf, xa_audio_ring->len);
#endif
